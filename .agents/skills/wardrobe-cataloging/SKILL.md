...
```
Wardrobe/
├── AGENTS.md                            ← thin project pointer (points at this skill)
├── .Codex/skills/wardrobe-cataloging/
...
1. **Find the next 10 unprocessed images.** Whether a file is processed
   ...
   "Item Image Name" column of `working/wardrobe_catalog.csv`
   (pipe-separated names count). Do not rely on session state,
   transcripts, or memory.
   ```
   python3 .Codex/skills/wardrobe-cataloging/scripts/find_unprocessed.py 10
   ```
...
3. **Deduplicate against the existing catalog.** Match on color +
   ...
   brand. If two images show the same garment (one flat, one worn),
   merge them into a single catalog row and pipe-separate the
   filenames in "Item Image Name". If unsure whether two near-similar
   items are the same, **do not silently merge** — list as separate
   IDs and add a "POSSIBLE DUPLICATE of [ID]" row to
...
4. **Construct rows** for all three CSVs using the schemas in
   ...
   validated.
5. **Append the rows** via the library:
   ```python
   import sys
   from pathlib import Path
   sys.path.insert(0, ".Codex/skills/wardrobe-cataloging/scripts")
...
6. **Verify integrity:**
   ```
   python3 .Codex/skills/wardrobe-cataloging/scripts/append_batch.py --sanity-check
   python3 .Codex/skills/wardrobe-cataloging/scripts/verify_csvs.py
   ```
...
## Schema — wardrobe_catalog.csv (40 columns, in order)
...
4. Image Type — Flat lay / Worn photo / Shoe / Accessory / Duplicate angle
5. Mapped Item ID(s) — Item ID this photo informs (comma-separated for multiple)
...
- **`Item Image Name` is the dedup key.** It is the single source of
  truth for "is this image processed?" — do not rely on Image Number
  or session state.
...
- `scripts/find_unprocessed.py` is authoritative for what's left. If
  it says 0 unprocessed, Pass 1 is done.
